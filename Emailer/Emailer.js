...
function main(){
		newBuildExists().then(function(res){			
			if(res){
				...
				url = url.replace(/(\r\n|\n|\r)/gm,"");
				var number = (""+current).replace(/(\r\n|\n|\r)/gm,"");
				getRequest(url).then(function(res){
					var perfJobs = res;
					// setup e-mail data with unicode symbols
					var mailOptions = {
					    from: 'Performance Reporter <performancereporter@mulesoft.com>', // sender address
					    to: 'naseem.alnaji@mulesoft.com, luciano.gandini@mulesoft.com, wai.ip@mulesoft.com, mohammed.abouzahr@mulesoft.com', // list of receivers
					    subject: 'PERF_CI #'+number+' Report', // Subject line
					    html: '<p><b>Performance Report for PERF_CI Run #'+number+':</b></p>' // html body
					};
					var timestampURL = 'http://mule-perflab06.managed.contegix.com:8080/job/PERF_CI/'+number+'/api/json?tree=timestamp';
					getRequestWithAuth(timestampURL, username, password).then(function(res){
						var date = new Date(parseFloat(res['timestamp']) - 28800000);
						var timeHTML = size('18pt', (date.toUTCString()).substring(0,date.toUTCString().length-3));
						mailOptions.html += '<p>Build completed at '+ timeHTML +'</p>';
					});
